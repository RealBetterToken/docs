...
要在 Cursor 接入 BetterToken 自定义 OpenAI API，请在模型设置中填写 **GPT 分组**API Key，开启 Override OpenAI Base URL，并填入 `{{openai-base-url}}`。然后从模型广场添加当前可用的 GPT 模型 ID。API Key、Base URL 和 Model ID 必须属于同一个 provider。
