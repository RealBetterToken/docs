...
在 Cline 中选择 **OpenAI Compatible**，将 **Base URL** 填为 `{{openai-base-url}}`，粘贴你的 BetterToken API Key，再填写 **GPT 提供商**中的 Model ID。不要使用 Claude Code 的 Anthropic-compatible 地址，也不要选择官方 **OpenAI** provider。
...
| Model ID | <a href={"{{model-plaza-url}}"}>模型广场</a>中 **GPT 提供商**的模型 ID |
...
<Steps>
  <Step title="准备 API Key 和模型 ID">
    创建 API Key 后，从 <a href={"{{model-plaza-url}}"}>模型广场</a>复制 **GPT 提供商**中当前可用的模型 ID。Model ID 必须原样填写，不能使用页面上的模型展示名称代替。
  </Step>
...
| `Model Not Found` | Model ID 是否属于 GPT 提供商 | 从模型广场重新复制当前可用的 Model ID，不要手动改写大小写或后缀 |
...
这套字段适用于 Cline 通过 BetterToken 的 OpenAI-compatible endpoint 调用 **GPT 提供商**模型。它不会修改 Cline Rules、Memory Bank、MCP 或本地工具权限；这些功能仍由 Cline 自己管理。
