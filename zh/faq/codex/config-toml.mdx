...
把用户级配置写入 `~/.codex/config.toml`，让 `model_provider = "custom"` 对应 `[model_providers.custom]`。使用 LLMEasy 时，`base_url` 填 `{{openai-base-url}}`，`wire_api` 填 `responses`，模型从 **GPT 分组**复制。API Key 推荐通过 `env_key` 指向本机环境变量，不要直接写进 TOML 或提交到仓库。
...
<Steps>
  <Step title="设置 API Key 环境变量">
    ...
    将 `YOUR_API_KEY` 替换为你的 LLMEasy API Key。长期使用时，把变量保存在你操作系统的安全环境配置中，不要写入项目仓库。
  </Step>
...
- [Codex 完整接入 LLMEasy 指南](/zh/ai-tools/codex)
