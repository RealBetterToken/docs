...
要在 Codex CLI 使用自定义 provider，请编辑用户级 `~/.codex/config.toml`。设置 `model_provider = "custom"`，添加 `[model_providers.custom]`，Base URL 填 `{{openai-base-url}}`，并设置 `wire_api = "responses"`。BetterToken API Key 通过 `env_key` 读取，Model ID 从 **GPT 提供商**复制。
...
<Steps>
  ...
  <Step title="写入 config.toml">
    ...
    `model` 只是示例。实际值请从<a href={"{{model-plaza-url}}"}>模型广场</a>复制 **GPT 提供商**中当前可用的模型 ID。
  </Step>
...
| `model` | 默认模型 ID | GPT 提供商中的当前可用 ID |
...
| Model not found | 模型 ID 不属于 GPT 提供商或已下线 | 从模型广场重新复制当前 Model ID |
