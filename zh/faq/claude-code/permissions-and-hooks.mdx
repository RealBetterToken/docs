...
这两项控制的是 Claude Code 在你电脑上的工具行为。接入 LLMEasy 只会改变模型 API 的 Base URL、API Key 和路由，不会绕过本地权限或替你运行 hooks。
...
| 接入 LLMEasy 后仍出现确认弹窗 | API provider 与本地权限是两套配置 | 保留 LLMEasy Base URL 配置，另外在 Claude Code 中设置精细 permissions |
| 想临时停用全部 hooks | 需要排除 hook 干扰 | 在设置中添加 `"disableAllHooks": true`；排查完成后删除或改回 `false` |

## 适用于 LLMEasy 的边界

LLMEasy 负责 Claude Code 的模型 API 接入、模型路由、余额和用量记录。下面这些行为仍由本地 Claude Code 控制：
...
如果问题是 `401`、Base URL 或模型映射，请查看 [Claude Code 接入 LLMEasy 指南](/zh/ai-tools/claude-code)。如果问题是命令确认或自动化脚本，请继续在本页的 permissions 与 hooks 中排查。
