...
BetterToken 现已支持 `claude-opus-5`。该模型适合复杂编码、长时间 Agent 工作流，以及围绕大型代码库展开的多步骤任务。接入时，请创建 `Claude` 分组的 API Key，使用 Base URL `{{anthropic-base-url}}`，并将模型 ID 设置为 `claude-opus-5`。
...
1. <a href={"{{register-url}}"}>注册并创建 API Key</a>，Key group 选择 `Claude`。
