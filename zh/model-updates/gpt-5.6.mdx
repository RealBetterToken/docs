...
LLMEasy 已上架 GPT-5.6 系列模型。你可以在<a href={"{{model-plaza-url}}"}>模型广场</a>查看当前可用的 **GPT 分组**模型 ID 和实时价格，并在 Codex、OpenCode、OpenClaw、Cursor、Cline、Zed 等外部工具中通过 OpenAI 兼容协议使用。
...
- **适合外部工具接入**：OpenAI Help Center 确认 Sol、Terra 和 Luna 均可通过 OpenAI API 使用；在 LLMEasy 中，请使用 **GPT 分组**和 `{{openai-base-url}}`
