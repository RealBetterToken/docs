...
description: "在 VS Code Codex 插件中打开共享的 config.toml，配置 LLMEasy Base URL、API Key 和 GPT 分组模型，并排查插件未读取配置的问题。"
icon: "/icons/codex-vscode.svg"
---

## 直接答案

VS Code Codex 插件没有独立的 Base URL 和 API Key 输入框。它与 Codex CLI 共享配置层，因此你需要通过插件右上角的齿轮打开 **Codex Settings → Open config.toml**，再在用户级配置中添加 LLMEasy provider。配置完成后执行 **Developer: Reload Window**，让插件读取新地址、密钥和模型。
...
| `base_url = "{{openai-base-url}}"` | 将模型请求发送到 LLMEasy 的 OpenAI-compatible / Responses API 入口 |
