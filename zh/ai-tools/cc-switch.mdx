---
title: "CC Switch 接入 LLMEasy 指南"
sidebarTitle: "CC Switch"
description: "在 CC Switch 中接入 LLMEasy，统一管理 Claude Code、Codex CLI、OpenCode 和 OpenClaw。"
---
...
- LLMEasy API Key（<a href={"{{register-url}}"}>注册并获取</a>）
...
- 给 Codex、OpenCode、OpenClaw 准备一个当前可用的 **GPT 分组**模型 ID。你可以从 <a href={"{{model-plaza-url}}"}>LLMEasy 模型广场</a> 复制，或在 CC Switch 里用 **获取模型** 直接从 `/v1/models` 拉取
...
<Note>
  LLMEasy 同时提供 Anthropic 和 OpenAI-compatible 两种接入模式。为了避免把 `{{anthropic-base-url}}` 和 `{{openai-base-url}}` 混在一起，建议在 CC Switch 里按应用分别创建 provider，而不是把 Claude Code 和 OpenAI-compatible 工具合并到一个统一供应商。
</Note>
...
<Tabs>
      ...
      <Step title="保存并切换">
        ...
        <Frame>
          <img
            ...
            alt="CC Switch 的 Codex provider 列表页，LLMEasy 已被选中，并显示为使用中。"
            style={{ borderRadius: '0.5rem' }}
          />
        </Frame>
      </Step>
  ...
  <Tab title="OpenCode" id="opencode">
    <Steps>
      ...
      <Step title="选择默认模型">
        优先点击 **获取模型**。如果需要手动填写，请直接使用 <a href={"{{model-plaza-url}}"}>LLMEasy 模型广场</a> 中当前可用的 **GPT 分组**模型 ID。
      </Step>
