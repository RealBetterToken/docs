---
title: "文生图（图片生成）"
description: "通过 LLMEasy 的 OpenAI 兼容接口调用 GPT Image 2，根据文本提示词生成图片。"
...
<Note>
  使用 `{{openai-base-url}}` 作为 `Base URL`。调用时通过 `Authorization: Bearer YOUR_API_KEY` 传入 LLMEasy API Key。
</Note>
