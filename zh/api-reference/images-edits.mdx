...
需要引导模型只编辑指定区域时，可以增加可选文件字段 `mask`。当前 BetterToken 后端只读取一个 PNG `mask` 文件，请不要重复提交 `mask[]`。

- 原图和 mask 必须使用相同的文件格式和像素尺寸。由于 mask 必须是 PNG，请将两个文件都保存为 PNG。每个文件必须小于 50 MB。
...
下面的最小 Pillow 检查会验证文件格式、尺寸、Alpha 通道，以及是否同时存在完全透明和完全不透明像素：
...
- 按蒙版编辑时只提交一个 PNG `mask`，并确认它与原图同尺寸且包含 Alpha 通道。
