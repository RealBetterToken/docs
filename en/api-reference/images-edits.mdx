...
description: "Generate or edit images from reference images with GPT Image 2 through LLMEasy's OpenAI-compatible API."
...
<Note>
  Use `{{openai-base-url}}` as the `Base URL`. Pass your LLMEasy API Key through `Authorization: Bearer YOUR_API_KEY`.
</Note>
