...
When connecting BetterToken to Cursor, use the OpenAI-compatible endpoint. External tools use the GPT Key group, so copy the current model ID from the model plaza (`{{model-plaza-url}}`).
...
- Do not keep using hardcoded Claude model names; use a current GPT Key group model ID from the model plaza (`{{model-plaza-url}}`)
