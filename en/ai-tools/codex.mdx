---
title: "Connect Codex to LLMEasy with a custom model"
sidebarTitle: "Codex"
description: "Connect LLMEasy to Codex with /v1 and OpenAI Responses."
icon: "code"
---

Connecting Codex to LLMEasy requires a custom model provider and API Key. The `base_url` must be `{{openai-base-url}}`, and the protocol is `OpenAI Responses`.
  ...
  <Tab title="Manual setup">
    <Steps>
      <Step title="Confirm the default models">
        The config below uses `gpt-5.5` as the default model and `gpt-5.4` as the review model. If you need to switch models, open the <a href={"{{model-plaza-url}}"}>LLMEasy model plaza</a> and copy a currently available model ID from the **GPT Key group**.
      </Step>
