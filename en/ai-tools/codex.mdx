  ...
  <Tab title="Manual setup">
    <Steps>
      <Step title="Copy a model ID from the model plaza">
        Open the BetterToken model plaza (`{{model-plaza-url}}`) and copy a currently available model ID from the **Codex group**. Replace every `YOUR_CODEX_MODEL_ID` below with that value.
      </Step>
