---
title: "Connect Claude Code to BetterToken with a custom model"
  ...
  <Tab title="Manual setup">
    <Steps>
      <Step title="Check your Key group">
        First confirm which BetterToken Key group you are using:
      ...
      <Step title="Edit the Claude Code config file">
        ...
        If you use the **Claude Key group**, replace `YOUR_API_KEY` with your BetterToken API Key:
      ...
      <Step title="Add model mappings for the GPT Key group">
        ...
        <Note>
          `gpt-5.4` is an example **GPT Key group** model ID. Use the current **GPT Key group** model IDs listed in the <a href={"{{model-plaza-url}}"}>BetterToken model plaza</a>. You can map all three tiers to the same model or choose different ones.
        </Note>
      </Step>
...
<Steps>
  ...
  <Step title="Add model mappings for the GPT Key group">
    ...
    <Note>
      `gpt-5.4` is only an example **GPT Key group** model ID. Copy the current model ID from the <a href={"{{model-plaza-url}}"}>BetterToken model plaza</a>. When the extension uses the Opus tier by default, it will resolve through `ANTHROPIC_DEFAULT_OPUS_MODEL`.
    </Note>
  </Step>
