---
title: "Connect Claude Code to LLMEasy with a custom model"
sidebarTitle: "Claude Code"
description: "Connect LLMEasy to Claude Code with ANTHROPIC_BASE_URL and no /v1 suffix."
icon: "terminal"
---
  ...
  <Tab title="Manual setup">
    <Steps>
      ...
      <Step title="Add model mappings for the GPT Key group">
        ...
        <Note>
          `gpt-5.4` is an example **GPT Key group** model ID. Use the current **GPT Key group** model IDs listed in the <a href={"{{model-plaza-url}}"}>LLMEasy model plaza</a>. You can map all three tiers to the same model or choose different ones.
        </Note>
      </Step>
...
<Steps>
  ...
  <Step title="Add model mappings for the GPT Key group">
    ...
    <Note>
      `gpt-5.4` is only an example **GPT Key group** model ID. Copy the current model ID from the <a href={"{{model-plaza-url}}"}>LLMEasy model plaza</a>. When the extension uses the Opus tier by default, it will resolve through `ANTHROPIC_DEFAULT_OPUS_MODEL`.
    </Note>
  </Step>
