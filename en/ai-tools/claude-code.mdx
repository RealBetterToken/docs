  ...
  <Tab title="Manual setup">
    <Steps>
      ...
      <Step title="Add model mappings for the GPT Key group">
        ...
        <Note>
          `gpt-5.4` is an example **GPT Key group** model ID. Use the current **GPT Key group** model IDs listed in the BetterToken model plaza (`{{model-plaza-url}}`). You can map all three tiers to the same model or choose different ones.
        </Note>
      </Step>
...
<Steps>
  ...
  <Step title="Add model mappings for the GPT Key group">
    ...
    <Note>
      `gpt-5.4` is only an example **GPT Key group** model ID. Copy the current model ID from the BetterToken model plaza (`{{model-plaza-url}}`). When the extension uses the Opus tier by default, it will resolve through `ANTHROPIC_DEFAULT_OPUS_MODEL`.
    </Note>
  </Step>
