---
title: "How to connect Dify to LLMEasy custom models"
sidebarTitle: "Dify"
description: "Add a LLMEasy GPT Key group model through Dify's official OpenAI-API-compatible plugin with the API Base URL, API Key, and model ID."
icon: "layer-group"
---

To connect Dify to LLMEasy, install the official **OpenAI-API-compatible** model plugin and add a custom LLM configuration for each GPT Key group model you want to use. Set the API Base URL to `{{openai-base-url}}`.
...
This is expected. A chat LLM cannot replace an embedding model. This guide does not confirm LLMEasy embedding or rerank endpoints for Dify; configure a supported embedding provider separately for Knowledge.
