...
description: "Connect OpenCode to BetterToken OpenAI-compatible Chat Completions with /connect, opencode.json, an API Key, Base URL, and Model ID. Includes setup guidance."
icon: "brackets-curly"
---

OpenCode connects to BetterToken with `@ai-sdk/openai-compatible`. Name the OpenCode provider **BetterToken** or **OpenAI Compatible**. GPT, Kimi, and GLM are model providers inside BetterToken, not OpenCode provider names. Stay on this page for general API Key, Base URL, or `opencode.json` setup. Use the dedicated [GLM guide](/en/faq/opencode/glm-api) or [Kimi K3 guide](/en/faq/opencode/kimi-k3-api) only when you need to select and verify that specific model.
