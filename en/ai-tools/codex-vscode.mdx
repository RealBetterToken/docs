...
description: "Open the shared config.toml from the VS Code Codex extension, configure LLMEasy Base URL, API Key, and GPT Key group model, and fix config loading issues."
icon: "/icons/codex-vscode.svg"
---

## Direct answer

The VS Code Codex extension has no separate Base URL or API Key fields. It shares configuration layers with Codex CLI. Click the gear icon, choose **Codex Settings → Open config.toml**, and add the LLMEasy provider to your user config. Then run **Developer: Reload Window** so the extension reads the new endpoint, Key, and model.
...
- You have a LLMEasy API Key (<a href={"{{register-url}}"}>sign up and get one</a>)
...
| `base_url = "{{openai-base-url}}"` | Sends model requests to LLMEasy's OpenAI-compatible / Responses API endpoint |
