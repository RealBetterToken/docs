---
title: "FAQ"
description: "Find answers about BetterToken accounts, billing, usage, API Keys, Base URLs, supported tools, model access, common errors, and connection troubleshooting."
---
...
- [How do I configure GPT-6 Astra in Codex API mode?](/en/faq/codex/gpt-5.5-api-mode)
...
<AccordionGroup>
  <Accordion title="How does BetterToken charge usage?">
    BetterToken charges by actual token usage and follows Anthropic / OpenAI pricing without adding an extra markup.

    See the <a href={"{{pricing-url}}"}>pricing page</a> for details.
  </Accordion>

  <Accordion title="Can I get a refund after signup?">
    Contact support if you need a refund for a paid top-up.
  </Accordion>
...
<AccordionGroup>
  ...
  <Accordion title="Do I need to specify a model when using Claude Code?">
    ...
    With the **Claude provider**, only set `ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN`. You do **not** need model variables.

    With the **GPT provider**, add Claude Code model mappings through `ANTHROPIC_DEFAULT_HAIKU_MODEL`, `ANTHROPIC_DEFAULT_SONNET_MODEL`, and `ANTHROPIC_DEFAULT_OPUS_MODEL`. Copy the model ID from the current **GPT provider** in the <a href={"{{model-plaza-url}}"}>model plaza</a>.
  </Accordion>

  <Accordion title="Do I need to specify a model when using Codex?">
    Yes. Fill in a model ID copied from the **GPT provider** in the <a href={"{{model-plaza-url}}"}>model plaza</a>.
  ...
  <Accordion title="Do I need to specify a model when using other tools like Cursor?">
    Yes. External tools such as Cursor, Cline, Zed, and OpenClaw now use the **GPT provider**. Copy the current model ID from the <a href={"{{model-plaza-url}}"}>model plaza</a>.
