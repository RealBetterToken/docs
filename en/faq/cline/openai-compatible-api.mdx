...
In Cline, select **OpenAI Compatible**, set **Base URL** to `{{openai-base-url}}`, paste your BetterToken API Key, and enter a Model ID from the **GPT provider**. Do not use the Anthropic-compatible address for Claude Code, and do not select the official **OpenAI** provider.
...
| Model ID | A model ID from the **GPT provider** in the <a href={"{{model-plaza-url}}"}>model plaza</a> |
...
<Steps>
  <Step title="Prepare the API Key and Model ID">
    Create an API Key, then copy a currently available Model ID from the **GPT provider** in the <a href={"{{model-plaza-url}}"}>model plaza</a>. Enter the ID exactly as shown instead of using the marketing display name.
  </Step>
...
| `Model Not Found` | Whether the Model ID belongs to the GPT provider | Copy an available ID from the model plaza without changing its case or suffix |
...
These values connect Cline to **GPT provider** models through the BetterToken OpenAI-compatible endpoint. They do not change Cline Rules, Memory Bank, MCP, or local tool permissions.
