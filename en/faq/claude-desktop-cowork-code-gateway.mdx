...
These issues usually come from two separate causes: the desktop Gateway auth scheme does not match the API Key group, or the bundled Claude Code version in Claude Desktop has not finished updating.
...
**Root cause**: **Gateway auth scheme** does not match the API Key group.

In Claude Desktop Gateway settings, choose the auth scheme based on the API Key group:
