...
These issues usually come from two separate causes: the desktop Gateway auth scheme does not match the Gateway, or the bundled Claude Code version in Claude Desktop has not finished updating.
...
**Root cause**: **Gateway auth scheme** is set to `bearer`.

The BetterToken Anthropic-compatible endpoint expects requests to use `x-api-key: YOUR_API_KEY`. If Claude Desktop uses `bearer`, it sends `Authorization: Bearer YOUR_API_KEY`, and Cowork mode may fail Gateway authentication.

Claude Code CLI can still work because it sends the key with `x-api-key` by default.
...
<Steps>
  ...
  <Step title="Change the auth scheme">
    Change **Gateway auth scheme** from `bearer` to `x-api-key`.
  </Step>
...
|---------|-------------------|-----|
| Cowork mode does not work | **Gateway auth scheme** is set to `bearer` | Change it to `x-api-key`, then click **Apply locally** |
