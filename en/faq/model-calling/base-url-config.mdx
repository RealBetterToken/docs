...
| --- | --- |
| Claude Code | `{{anthropic-base-url}}`, with a Claude group API Key |
| Claude Desktop Gateway | Follow the Gateway page, including the auth scheme |
| Codex CLI | `{{openai-base-url}}`, with a GPT group API Key |
| Cursor | `{{openai-base-url}}`, with a GPT group API Key |
| Cline | `{{openai-base-url}}`, using an OpenAI Compatible provider |
| OpenCode / OpenClaw | `{{openai-base-url}}`, with a GPT group API Key |
...
4. Check whether the Model ID comes from the <a href={"{{model-plaza-url}}"}>model plaza</a> and belongs to the current Key group.
