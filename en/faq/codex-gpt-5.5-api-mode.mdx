...
You can add a local `custom-models.json` file so Codex CLI can recognize `gpt-5.5`, then reference it from `~/.codex/config.toml`. If a later Codex CLI version supports `gpt-5.5` natively, prefer the built-in Codex CLI setup.
