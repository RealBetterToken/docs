...
description: "Understand Cursor MCP, project and global configuration, mcp.json, tool approval, and how MCP differs from model API setup. Includes examples and fixes."
---
...
BetterToken is the model API layer. Cursor MCP is the tool layer. You can use both, but they do not replace each other.

If your goal is to switch model routing, configure `{{openai-base-url}}` and a **GPT provider** model. If your goal is to let Cursor call external tools, configure MCP.
