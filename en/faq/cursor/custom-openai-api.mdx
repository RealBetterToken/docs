...
To connect BetterToken as a Custom OpenAI API in Cursor, enter an BetterToken API Key, enable Override OpenAI Base URL, and set it to `{{openai-base-url}}`. Then add a current GPT Model ID from the model plaza. The API Key, Base URL, and Model ID must all belong to the same provider.
