...
If a project uses LLMEasy, `CLAUDE.md` can remind Claude Code that Claude Code uses the Anthropic-compatible `{{anthropic-base-url}}`, while Codex and external tools use the OpenAI-compatible `{{openai-base-url}}`.
