...
description: "Learn what Claude Code is, how it works with a codebase, and how context, tool permissions, MCP, and API providers fit together. Includes setup guidance."
---
...
When Claude Code uses a custom API Key and Base URL, it can send model requests through an Anthropic-compatible API gateway. BetterToken belongs to that model API layer and can help manage API Keys, balance, and usage history.

BetterToken does not replace Claude Code permissions, file access controls, hooks, MCP config, or project rules. Those remain part of the Claude Code workflow.
