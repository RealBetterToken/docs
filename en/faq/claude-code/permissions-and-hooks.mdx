...
These settings control local tools on your computer. LLMEasy changes the model API Base URL, API Key, and routing only; it does not bypass local permissions or run hooks for you.
...
| Prompts remain after connecting LLMEasy | API provider and local permissions are separate | Keep the LLMEasy Base URL and configure narrow Claude Code permissions separately |
...
LLMEasy handles Claude Code model API access, model routing, balance, and usage. Claude Code still controls:
...
- [Claude Code setup with LLMEasy](/en/ai-tools/claude-code)
