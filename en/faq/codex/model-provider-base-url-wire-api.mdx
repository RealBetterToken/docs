...
LLMEasy can be defined as a custom provider such as `[model_providers.custom]`. Use `{{openai-base-url}}` as the `base_url` and copy the model ID from the **GPT Key group**.
