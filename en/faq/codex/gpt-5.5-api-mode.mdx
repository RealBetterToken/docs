...
<Warning>
  This page only adds a local model catalog. If `[model_providers.{{codex-provider-id}}]` already works, keep that provider and its authentication fields; add only `model_catalog_json` and the model selections. The complete example below uses `env_key = "BETTERTOKEN_API_KEY"` with `requires_openai_auth = false`. Set that environment variable with the manual steps in the regular [Codex setup guide](/en/ai-tools/codex) before restarting Codex.
</Warning>
...
6. `env_key` points to `BETTERTOKEN_API_KEY`, and that variable is available to the process that starts Codex
