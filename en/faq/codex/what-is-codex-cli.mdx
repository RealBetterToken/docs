---
title: "Codex CLI: what it is, how it works, and how App and Web differ"
sidebarTitle: "What is Codex CLI"
description: "Learn how Codex CLI works in a local terminal, accesses repositories, runs commands, applies project rules, and differs from Codex App and Codex Web workflows."
---

Codex CLI is OpenAI's local command-line coding agent. It runs in your terminal, works with the selected workspace, reads and edits files, runs commands, and applies your sandbox and approval settings. Unlike Codex App, its primary interface is the terminal. Unlike Codex Web, it runs in your local environment rather than a browser or remote runtime.
...
## CLI, App, and Web at a glance

| Option | Where it runs | Choose it when |
| --- | --- | --- |
| Codex CLI | Local terminal and workspace | You need commands, files, and `config.toml` setup |
| Codex App | Desktop application | You need graphical sessions, plugins, or Remote Control |
| Codex Web | Browser or remote environment | The task should run in a cloud or remote environment |

See [Codex CLI vs Codex App vs Codex Web](/en/faq/codex/codex-cli-vs-app-vs-web) for a detailed comparison.
...
When you use a custom OpenAI-compatible provider, BetterToken can act as the model API layer for Codex CLI. It handles the API Key, Base URL, model routing, balance, and usage records.

BetterToken does not replace Codex CLI's local sandbox, approvals, `AGENTS.md`, or command execution policy.

For general Codex API access and product use cases, see the <a href="https://bettertoken.ai/ru/codex-api/">BetterToken Codex API page</a>. This FAQ focuses on Codex CLI and its local workflow.
