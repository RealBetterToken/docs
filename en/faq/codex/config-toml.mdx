...
description: "Configure Codex config.toml for a custom provider: set model_provider, Base URL, API Key env_key, wire_api responses, and fix 401, 404, or model errors."
---

## Direct answer

To use a custom provider in Codex CLI, edit the user-level `~/.codex/config.toml`. Set `model_provider = "custom"`, define `[model_providers.custom]`, use `{{openai-base-url}}` as the Base URL, and set `wire_api = "responses"`. Read the BetterToken API Key through `env_key`, and copy the Model ID from the **GPT provider**.
...
<Steps>
  ...
  <Step title="Edit config.toml">
    ...
    `model` is an example. Copy a currently available Model ID from the **GPT provider** in the <a href={"{{model-plaza-url}}"}>model plaza</a>.
  </Step>
...
| `model` | Default Model ID | A current GPT provider ID |
...
| Model not found | The Model ID is unavailable or not in the GPT provider | Copy a current ID from the model plaza |
