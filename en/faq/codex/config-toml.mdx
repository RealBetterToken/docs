...
To use a custom provider in Codex CLI, edit the user-level `~/.codex/config.toml`. Set `model_provider = "custom"`, define `[model_providers.custom]`, use `{{openai-base-url}}` as the Base URL, and set `wire_api = "responses"`. Read the BetterToken API Key through `env_key`, and copy the Model ID from the **GPT Key group**.
