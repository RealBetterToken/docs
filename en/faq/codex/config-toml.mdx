...
Put user-level settings in `~/.codex/config.toml` and make `model_provider = "custom"` point to `[model_providers.custom]`. For BetterToken, use `{{openai-base-url}}` as `base_url`, `responses` as `wire_api`, and a Model ID from the **GPT Key group**. Use `env_key` to read the API Key from a local environment variable instead of storing it in TOML or a repository.
