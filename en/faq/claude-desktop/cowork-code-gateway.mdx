...
description: "Troubleshoot Claude Desktop Cowork and Code mode after connecting a BetterToken Gateway by checking mapping, routing, certificates, settings, and restarts."
---
...
These issues usually come from two separate causes: the desktop Gateway auth scheme does not match the provider, or the bundled Claude Code version in Claude Desktop has not finished updating.

If you have not configured the Gateway yet, start with [Use BetterToken API in Claude Desktop](/en/faq/claude-desktop-bettertoken-api).

## Fix Cowork mode first

**Root cause**: **Gateway auth scheme** does not match the provider.

In Claude Desktop Gateway settings, choose the auth scheme based on the provider:

- If the BetterToken API Key is valid, set **Gateway auth scheme** to `x-api-key`
- If the BetterToken API Key is valid, set **Gateway auth scheme** to `bearer`
...
<Steps>
  ...
  <Step title="Change the auth scheme">
    Choose **Gateway auth scheme** based on the provider: use `x-api-key` for a **Claude provider** and `bearer` for a **GPT provider**.
  </Step>
...
|---------|-------------------|-----|
| Cowork mode does not work | **Gateway auth scheme** does not match the provider | Use `x-api-key` for a **Claude provider** or `bearer` for a **GPT provider**, then click **Apply locally** |
