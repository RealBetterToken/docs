...
This model uses OpenAI-compatible Chat Completions. Use the configuration below in an external tool that supports this protocol.
...
4. Set the Base URL to `{{openai-base-url}}` and use the exact temporary Model ID.
...
This model currently supports Chat Completions on BetterToken. Use `{{openai-base-url}}` in a compatible external tool. Claude Code and the Anthropic Messages API are not supported for this model.
