...
BetterToken has added the `gpt-image-2` image generation model entry. Check the model plaza (`{{model-plaza-url}}`) for the currently available model ID, then call it through the OpenAI-compatible image generation API.
...
Copy the current model ID from the model plaza (`{{model-plaza-url}}`). Do not use ChatGPT UI mode names as API parameters.
