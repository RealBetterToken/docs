...
description: "Claude Opus 5 is available on BetterToken as claude-opus-5 for complex coding, long-running agent workflows, and context windows up to 1M tokens for teams."
---

Release date: July 24, 2026

BetterToken now supports `claude-opus-5`. The model is designed for complex coding, long-running agent workflows, and multi-step work across large codebases. To connect it, create a BetterToken API Key, select the `Claude` provider, use `{{anthropic-base-url}}` as the Base URL, and select `claude-opus-5` as the model ID.
...
1. <a href={"{{register-url}}"}>Register and create a BetterToken API Key</a>, then select the `Claude` provider.
