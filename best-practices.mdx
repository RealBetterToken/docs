...
如果你同时使用过 Anthropic 官方 API 或其他中转服务，旧的环境变量可能会覆盖你的配置，导致请求发送到错误的地址。
...
同一个 API Key 可以同时在 Claude Code、Codex CLI、Cursor 等多个工具中使用。额度共享，方便统一管理。
...
2. **确认 base_url 正确** — 应为 `https://www.BetterToken.ai/v1`（注意是 `BetterToken.ai`）
