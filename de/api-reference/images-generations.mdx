...
- **Ein Bild aus Text generieren und das Ergebnis speichern:** Bleibe auf dieser Seite. Beginne mit dem `curl`-Beispiel und verwende anschließend **Bild speichern**, um `data[0].b64_json` zu dekodieren.
- **Ein vorhandenes Bild bearbeiten:** Verwende [Image to image](/de/api-reference/images-edits). Dabei werden eine oder mehrere Quelldateien als `multipart/form-data` gesendet.
- **Modell und Parameter wählen und das Budget schätzen:** Prüfe aktuelle Model IDs und Preise in der <a href={"{{model-plaza-url}}"}>model plaza</a> und verwende unten **Empfohlene Werte** für die Request-Parameter. Der [englische Artikel zum ersten GPT-Image-2-Request](https://bettertoken.ai/en/blog/gpt-image-2-first-api-request/) erläutert den vollständigen Request-Response-Ablauf.
