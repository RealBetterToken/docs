---
title: "Настройка кастомной модели OpenClaw через LLMEasy"
sidebarTitle: "OpenClaw"
description: "Подключите LLMEasy к OpenClaw, мультиплатформенному AI agent gateway."
icon: "paw"
---
...
<Tabs>
  <Tab title="Автоматическая настройка">
    Можно запустить автоматический скрипт LLMEasy. Подготовьте API Key LLMEasy до начала. Скрипту нужен `Node.js`. Если API Key или модель не переданы явно, скрипт запросит их интерактивно, когда это поддерживается. После завершения скрипта перейдите к разделу «Как применить» ниже.
  ...
  <Tab title="Ручная настройка">
    <Steps>
      ...
      <Step title="Добавьте credential LLMEasy через auth flow">
        ```bash
        openclaw models auth add
        ```
      </Step>

      <Step title="Добавьте или проверьте provider LLMEasy">
      ...
      <Step title="Extra `headers` обычно не нужны">
        При подключении LLMEasy обычно не нужен дополнительный `User-Agent`.
...
<Steps>
  ...
  <Step title="При необходимости проверьте available model picker">
    ...
    Если LLMEasy там не появляется, проверьте:
...
2. Если provider LLMEasy отсутствует, добавь provider с Base URL `{{openai-base-url}}` и `api: "openai-responses"`.
...
- Если в текущей конфигурации еще нет модели LLMEasy, попроси у меня model ID из группы **GPT** в LLMEasy model plaza.
```
