...
- BetterToken API Key（[注册并获取](https://www.bettertoken.ai/register)）
- Claude Code 使用 Anthropic 协议：`Base URL` 填 `https://www.bettertoken.ai`
- Codex CLI、OpenCode、OpenClaw 使用 OpenAI-compatible 协议：`Base URL` 填 `https://www.bettertoken.ai/v1`
- 给 Codex CLI、OpenCode、OpenClaw 准备一个当前可用的 **Codex 分组**模型 ID。你可以从 BetterToken [模型广场](https://www.bettertoken.ai/pricing) 复制，或在 CC Switch 里用 **获取模型** 直接从 `/v1/models` 拉取
...
<Tabs>
  ...
  <Tab title="Codex CLI" id="codex-cli">
    <Steps>
      <Step title="切到 Codex CLI 并添加 provider">
        打开 CC Switch，切到 **Codex CLI**，点击 **Add Provider**。如果界面先让你选预设，优先选择 **OpenAI Compatible** 或 **Custom** 这一类自定义 OpenAI-compatible provider。
        ...
        <Frame>
          <img
            ...
            alt="CC Switch 的 Codex CLI 页面，顶部选中了 Codex CLI，右上角加号按钮用于新增 provider。"
            style={{ borderRadius: '0.5rem' }}
          />
        </Frame>
      </Step>
      ...
      <Step title="填写基础字段">
        ...
        <Frame>
          <img
            ...
            alt="CC Switch 的 Codex CLI provider 编辑页，标出了 Provider Name、API Key 和 Base URL 的填写位置。"
            style={{ borderRadius: '0.5rem' }}
          />
        </Frame>
      </Step>
      ...
      <Step title="选择默认模型">
        ...
        <Frame>
          <img
            ...
            alt="CC Switch 的 Codex CLI provider 编辑页，展示了获取模型列表按钮以及模型下拉选择框。"
            style={{ borderRadius: '0.5rem' }}
          />
        </Frame>
      </Step>
      ...
      <Step title="保存并切换">
        ...
        <Frame>
          <img
            ...
            alt="CC Switch 的 Codex CLI provider 列表页，BetterToken 已被选中，并显示为使用中。"
            style={{ borderRadius: '0.5rem' }}
          />
        </Frame>
      </Step>
  ...
  <Tab title="OpenCode" id="opencode">
    <Steps>
      ...
      <Step title="选择默认模型">
        优先点击 **获取模型**。如果需要手动填写，请直接使用 BetterToken [模型广场](https://www.bettertoken.ai/pricing) 中当前可用的 **Codex 分组**模型 ID。
      </Step>
...
- Codex CLI、OpenCode、OpenClaw 默认模型请使用 **Codex 分组**模型 ID，不要沿用 Claude 风格模型名
...
- Codex CLI 细节配置：[Codex CLI](/ai-tools/codex)
