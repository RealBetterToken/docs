...
- 给 Codex、OpenCode、OpenClaw 准备一个当前可用的 **GPT 分组**模型 ID。你可以从 BetterToken 模型广场 (`{{model-plaza-url}}`) 复制，或在 CC Switch 里用 **获取模型** 直接从 `/v1/models` 拉取
...
<Tabs>
  ...
  <Tab title="OpenCode" id="opencode">
    <Steps>
      ...
      <Step title="选择默认模型">
        优先点击 **获取模型**。如果需要手动填写，请直接使用 BetterToken 模型广场 (`{{model-plaza-url}}`) 中当前可用的 **GPT 分组**模型 ID。
      </Step>
