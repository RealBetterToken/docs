...
- 如果 API Key 对应 **Codex 分组**，**Gateway auth scheme** 选择 `bearer`
...
<Steps>
  ...
  <Step title="切换认证方式">
    按 API Key 分组选择 **Gateway auth scheme**：**Claude 分组** 使用 `x-api-key`，**Codex 分组** 使用 `bearer`。
  </Step>
...
| 现象 | 常见根因 | 解决方式 |
|------|----------|----------|
| Cowork 模式不可用 | **Gateway auth scheme** 与 API Key 分组不匹配 | **Claude 分组** 选择 `x-api-key`，**Codex 分组** 选择 `bearer`，并点击 **Apply locally** |
