---
title: "Codex CLI config.toml 如何配置自定义 provider？"
sidebarTitle: "config.toml"
description: "配置 Codex CLI 自定义 provider：文件位置、最小可用示例、API Key 环境变量、配置层级、验证与常见错误。"
---

## 直接答案

把用户级配置写入 `~/.codex/config.toml`，让 `model_provider = "custom"` 对应 `[model_providers.custom]`。使用 BetterToken 时，`base_url` 填 `{{openai-base-url}}`，`wire_api` 填 `responses`，模型从 **GPT 分组**复制。API Key 推荐通过 `env_key` 指向本机环境变量，不要直接写进 TOML 或提交到仓库。
...
<Note>
  如果你的目标是在使用第三方 API 时继续保留 Codex App 官方登录态、插件和 Remote Control，不要直接套用普通鉴权配置。请改用 [保留 Codex 官方登录态和会话历史](/faq/codex/official-login-third-party-api) 的专用方案。
</Note>
...
- [Codex 完整接入 BetterToken 指南](/ai-tools/codex)
- [VS Code Codex 插件如何配置自定义 Base URL 和 API Key？](/ai-tools/codex-vscode)
