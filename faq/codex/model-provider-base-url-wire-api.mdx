...
这表示：Codex 使用名为 `bettertoken` 的 provider，把模型请求发送到 `{{openai-base-url}}`，并按 Responses API 形态通信。
...
BetterToken 可以作为 `[model_providers.bettertoken]` 这样的自定义 provider。`base_url` 使用 `{{openai-base-url}}`，模型 ID 从 **GPT 分组**复制。

在 LLMEasy 区域站中，同一个配置概念对应 LLMEasy 的 provider 名称和区域 Base URL。
