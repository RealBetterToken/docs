...
Эти настройки управляют локальными инструментами на вашем компьютере. LLMEasy меняет только Base URL, API Key и маршрутизацию model API; он не обходит локальные permissions и не запускает hooks вместо Claude Code.
...
| После подключения LLMEasy остались запросы подтверждения | API provider и локальные permissions независимы | Сохраните Base URL LLMEasy и отдельно настройте точные permissions Claude Code |
...
LLMEasy отвечает за доступ Claude Code к model API, маршрутизацию моделей, баланс и usage. Claude Code по-прежнему управляет:
...
- [Настройка Claude Code с LLMEasy](/ai-tools/claude-code)
