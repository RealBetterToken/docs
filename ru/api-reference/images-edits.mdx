...
description: "Генерируйте или редактируйте изображения по reference images через GPT Image2 и OpenAI-совместимый API BetterToken."
openapi: "POST /v1/images/edits"
---

`POST /v1/images/edits`

Endpoint image-to-image использует тело запроса `multipart/form-data`. Загрузите одно или несколько reference images и передайте prompt и параметры как поля формы.
...
<Tip>
  В Playground справа можно заполнить поля формы, загрузить reference images и отправить запрос напрямую на `{{images-edits-url}}`.
</Tip>
...
Для одного reference image используйте `image`. Для нескольких reference images используйте повторяющиеся поля `image[]`.
...
- [Text to image](/ru/api-reference/images-generations)
