...
Ajoutez le champ de fichier facultatif `mask` pour orienter la modification vers une zone sélectionnée. Le point de terminaison BetterToken actuel lit un seul fichier PNG dans `mask`. N’envoyez pas plusieurs champs `mask[]`.

- L’image d’origine et le masque doivent utiliser le même format et les mêmes dimensions en pixels. Le masque devant être au format PNG, utilisez ce format pour les deux fichiers. Chaque fichier doit faire moins de 50 MB.
...
Cette vérification minimale avec Pillow contrôle le format, les dimensions, le canal Alpha et la présence de pixels entièrement transparents et opaques :
