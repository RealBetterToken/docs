...
- **Générer une image à partir d’un texte et enregistrer le résultat :** restez sur cette page. Commencez par l’exemple `curl`, puis utilisez **Enregistrer l’image** pour décoder `data[0].b64_json`.
- **Modifier une image existante :** utilisez [Image to image](/fr/api-reference/images-edits), qui envoie un ou plusieurs fichiers source au format `multipart/form-data`.
- **Choisir un modèle, vérifier les paramètres et estimer le budget :** consultez les Model ID et prix actuels dans le <a href={"{{model-plaza-url}}"}>model plaza</a>, puis utilisez **Valeurs recommandées** ci-dessous pour les paramètres. L’[article en anglais sur la première requête GPT Image 2](https://bettertoken.ai/en/blog/gpt-image-2-first-api-request/) donne le contexte complet de la requête et de la réponse.
