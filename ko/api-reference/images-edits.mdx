...
선택한 영역을 중심으로 편집하려면 선택 사항인 `mask` 파일 필드를 추가하세요. 현재 BetterToken endpoint는 하나의 `mask` PNG 파일만 읽습니다. `mask[]` 필드를 여러 번 보내지 마세요.

- 원본 이미지와 mask는 파일 형식과 픽셀 크기가 같아야 합니다. Mask는 PNG여야 하므로 두 파일 모두 PNG를 사용하세요. 각 파일은 50 MB보다 작아야 합니다.
...
이 최소 Pillow 검사는 형식, 크기, Alpha channel, 완전히 투명한 픽셀과 완전히 불투명한 픽셀을 확인합니다.
