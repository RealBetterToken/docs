...
- **텍스트로 이미지를 생성하고 결과 저장:** 이 페이지를 계속 사용하세요. `curl` 예제로 시작한 다음 **이미지 저장**에서 `data[0].b64_json`을 decode하세요.
- **기존 이미지 편집:** 하나 이상의 source file을 `multipart/form-data`로 보내는 [Image to image](/ko/api-reference/images-edits)를 사용하세요.
- **모델과 parameters를 선택하고 budget 추정:** 현재 Model ID와 가격은 <a href={"{{model-plaza-url}}"}>모델 광장</a>에서 확인하고 request parameters는 아래 **권장 값**을 참고하세요. 전체 request-response 흐름은 [영문 GPT Image 2 첫 요청 문서](https://bettertoken.ai/en/blog/gpt-image-2-first-api-request/)에서 확인할 수 있습니다.
