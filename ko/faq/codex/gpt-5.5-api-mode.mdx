...
<Warning>
  이 페이지는 local model catalog만 추가합니다. `[model_providers.{{codex-provider-id}}]`가 이미 작동한다면 해당 provider와 authentication fields를 유지하고 `model_catalog_json`과 model selections만 추가하세요. 아래 전체 예시는 `env_key = "BETTERTOKEN_API_KEY"`와 `requires_openai_auth = false`를 사용합니다. Codex를 다시 시작하기 전에 일반 [Codex 설정 가이드](/ko/ai-tools/codex)의 수동 설정 단계에 따라 이 environment variable을 설정하세요.
</Warning>
...
6. `env_key`가 `BETTERTOKEN_API_KEY`를 가리키고 Codex를 시작하는 process에서 해당 variable을 사용할 수 있음
